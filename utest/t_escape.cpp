...
{
    std::string dst;
    wwjson::BasicConfig<std::string>::EscapeString(
    ...
    dst.clear();
    wwjson::BasicConfig<std::string>::EscapeString(
        dst, "Quote\"Here\\Back", ::strlen("Quote\"Here\\Back"));
...
DEF_TAST(escape_table_utf8, "UTF-8 字符不被转义测试")
{
    ...
    wwjson::BasicConfig<std::string>::EscapeString(dst, utf8_str,
                                                   ::strlen(utf8_str));
...
DEF_TAST(escape_table_mapping, "转义表映射优化测试")
{
    ...
    constexpr size_t table_size =
        wwjson::BasicConfig<std::string>::kEscapeTable.size();
    COUT(table_size, size_t{128});

    // Test all standard C/C++ escape sequences
    COUT(wwjson::BasicConfig<std::string>::kEscapeTable['\a'],
         uint8_t{'a'}); // bell
    COUT(wwjson::BasicConfig<std::string>::kEscapeTable['\b'],
         uint8_t{'b'}); // backspace
    COUT(wwjson::BasicConfig<std::string>::kEscapeTable['\t'],
         uint8_t{'t'}); // tab
    COUT(wwjson::BasicConfig<std::string>::kEscapeTable['\n'],
         uint8_t{'n'}); // newline
    COUT(wwjson::BasicConfig<std::string>::kEscapeTable['\v'],
         uint8_t{'v'}); // vertical tab
    COUT(wwjson::BasicConfig<std::string>::kEscapeTable['\f'],
         uint8_t{'f'}); // form feed
    COUT(wwjson::BasicConfig<std::string>::kEscapeTable['\r'],
         uint8_t{'r'}); // carriage return
    COUT(wwjson::BasicConfig<std::string>::kEscapeTable['"'],
         uint8_t{'"'}); // double quote
    COUT(wwjson::BasicConfig<std::string>::kEscapeTable['\\'],
         uint8_t{'\\'}); // backslash
    COUT(wwjson::BasicConfig<std::string>::kEscapeTable['\0'],
         uint8_t{'0'}); // null

    // Test non-printable characters map to '.'
    COUT(wwjson::BasicConfig<std::string>::kEscapeTable[0x01],
         uint8_t{'.'}); // SOH
    COUT(wwjson::BasicConfig<std::string>::kEscapeTable[0x02],
         uint8_t{'.'}); // STX
    COUT(wwjson::BasicConfig<std::string>::kEscapeTable[0x03],
         uint8_t{'.'}); // ETX
    COUT(wwjson::BasicConfig<std::string>::kEscapeTable[0x7F],
         uint8_t{'.'}); // DEL
...
{
    using EscapeKeyBuilder =
        wwjson::GenericBuilder<std::string, EscapeKeyConfig>;
...
{
    using EscapeValueBuilder =
        wwjson::GenericBuilder<std::string, EscapeValueConfig>;
...
DEF_TAST(escape_std_ascii, "标准 ASCII 转义字符测试")
{
    ...
    dst.clear();
    const char non_printable[] = {0x01, 0x02, 0x03, 0x04,
                                  0}; // SOH, STX, ETX, EOT
