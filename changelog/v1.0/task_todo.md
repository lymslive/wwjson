...
## TODO:2025-12-12/3 字符串转义优化方案预测试
...
频繁 push_back 的性能问题主要可能不是触发重新分配，毕竟也有 reserve 提前检测容
量，而是每次要做安全检测容量是否够。所以用 std::vector 做缓冲区也会遇到频繁
push_back 的问题，该直接用裸内存缓冲区。
...
## TODO:2025-12-13/1 性能测试文档

分析 perf/ 目录下的性能测试用例，更新 perf/cases.md 文档的用例列表。

编译目标在 `/build-release/perf/pfwwjson`，请运行数次减少浮动误差，支持命令行
参数调控测试数据大小与循环次数。生成性能测试报告，写在 perf/report.md 文档中。
报告中记录时间，当前环境是 win10 WSL1(ubuntu20.4) ，及 gcc 版本 9.4.0 。
...
### DONE: 20251213~234430
只有 README 是差不多可用的。
usage 文档 AI 生成版不满意，得手写。

### DONE: 20251219~223930
完成初稿 usage.md

## TODO:2025-12-15/1 优化定点小数部分序列化

在 perf/p_design.cpp 文件中增加一个相对性能测试用例。
测试四位小数的定点小数的小数部分的去除尾部零的方法如何实现具有更好的性能。

当前 NumberWriter::WriteSmall 的后半部分，我觉得写得不好，想研究、测试有没有其
他更好的办法。

测试样例数据，[0.0001, 0.9999] 范围内的浮点数，随机生成，
复用命令行参数 --start 表示随机种子，--items 表示生成浮点数个数，每次 loop 需
要序列化的数量。

因为只想专门比较小数部分的序列化，也可以随机生成 [1, 9999] 的整数。在对比方法
中假设小数部分已经分离并放大为整数，目标是写入 `.` 及其后的 1 至 4 位整数数字
。

方法 A：预先写入本地 buffer[5]，再从尾部判断是否为 `0` ，把前面非 0 部分用
append 方法写入目标字符串；

方法 B：就是当前实现，通过多重 if 判断要写入哪些数字，正向一个个写入目标字符串
。

我预估方法 A 可能会更快点，毕竟 4 位小数的占比更大些，一次字符串调用可减少边界
判断的重复开销。

如果方法 A 确实有性能提升，则据此改写 NumberWriter::WriteSmall ；否则再不修改
wwjson.hpp 文件，再分析探索其他优化方法。

### DONE:20251215-113942

## TODO:2025-12-20/1 同步文档示例与单元测试

将 docs/usage.md 文档的每个 cpp 代码示例标注名称：
- 命名格式 `<!-- example:usage_章节编号_名称 -->`
- 已命名为 `NO_TEST` 不用再标注
- 命名需结合上下文根据其用意取个恰当的名字

文档示例命名的用途是同步单元测试，以便验证其正确性。
...
cpp 代码块问题：
- 本地测试中，每个代码块只有第一行的位置是对的，从第二开始缩进了很多，很难看。
  但是在 github action 运行部署后，在 github.io 上看代码缩进量又是正常的
- 代码仍然没有语法高亮着色，ccs 类名与 pandoc 生成的不一致。

### DONE: 20251221-011034

## TODO:2025-12-21/2 文档网页优化

代码缩进问题原因：body 在模板中有缩进，pandoc 本地低版本把整体内容都缩进，远程
高版本可能会对 pre code 行特殊处理，顶格不缩进美化排版 html.

最简单的解决办法是将 docs/template.html 的 `$body$` 顶格写，减少所有正文的缩进。
之前各种尝试解决该问题添加的冗余 css 都可删了。可以只留下 pre code 及其组合覆
盖 code 的边框。

首页也加侧边栏目录，还是简单统一风格好，精简 pandoc 元数据传参。

首页 index.md 曾想直接使用 README.md ，考虑后又觉得不完全合适，可适合精简内容
。两者该有什么不同侧重点？

utest perf 两个子目录的 README.md 及其用例列表 cases.md 又是否值得网页化？
