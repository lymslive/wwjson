...
struct RelativeTimerConcept
{
    ...
    bool
    methodVerify(); // Optional: Verification method (recommended to override)
 ...
 * a framework for comparing the performance of two methods (methodA and
 * methodB) in a derived class. It runs the methods alternately in batches and
 * calculates the relative performance ratio.
 *
 * @tparam Derived The derived class that implements methodA and methodB
 */
template <typename Derived> class RelativeTimer
{
  public:
    /**
     ...
     *
     * Executes methodA and methodB alternately in batches for the specified
     * number of loops. Returns the performance ratio (timeA/timeB). Includes a
     * verification phase to ensure functional correctness before performance
     * measurement.
     *
     * @param loop Total number of iterations for each method
     * @param batch Number of batches to divide the loops into (default: 10)
     * @param timeA_ms Optional pointer to store methodA's total execution time
     * in milliseconds
     * @param timeB_ms Optional pointer to store methodB's total execution time
     * in milliseconds
     * @return double Performance ratio (timeA/timeB). Returns NaN if
     * verification fails
     */
    double run(int loop, int batch = 10, double *timeA_ms = nullptr,
               double *timeB_ms = nullptr)
    {
        ...
        // Verification phase - check functional correctness before performance
        // testing
        ...
        if (timeA_ms)
            *timeA_ms = timeA * 1000.0;
        if (timeB_ms)
            *timeB_ms = timeB * 1000.0;
        ...
        if (timeB == 0.0)
        {
            ...
            return timeA == 0.0
                       ? 1.0
                       : 1000.0; // Large value indicating A is much slower
        }
        return timeA / timeB;
    }
    ...
    double runAndPrint(const std::string &testName,
                       ...
                       const std::string &methodBLabel, int loop,
                       int batch = 10)
    {
        ...
        std::cout << "Performance ratio (" << methodALabel << "/"
                  << methodBLabel << ") " << timeA_ms << " ms / " << timeB_ms
                  << " ms = " << ratio << std::endl;

        if (ratio < 0.95)
        {
            std::cout << methodALabel << " is " << (1.0 / ratio - 1.0) * 100
                      << "% faster" << std::endl;
        }
        else if (ratio > 1.05)
        {
            std::cout << methodBLabel << " is " << (ratio - 1.0) * 100
                      << "% faster" << std::endl;
        }
        else
        {
