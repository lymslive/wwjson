 ...
 * 
 * Keys will be "k" + integer, values will be the sub-json string: {"key":"value"}
 * Uses AddMemberEscape for explicit escaping
 * 
 * @param dst Output string to store generated JSON
 * @param start Starting value for the sequence
 * @param count Number of key-value pairs to generate
...
void BuildStringArray(std::string& dst, int start, int count) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        yyjson_mut_val *str = yyjson_mut_strncpy(doc, str_val.c_str(), str_val.length());
...
void BuildStringObject(std::string& dst, int start, int count) {
    ...
    for (int i = 0; i < count; i++) {
        std::string value = std::to_string(start + i);
        std::string key = "k" + value;
        yyjson_mut_val *key_str = yyjson_mut_strncpy(doc, key.c_str(), key.length());
        yyjson_mut_val *val_str = yyjson_mut_strncpy(doc, value.c_str(), value.length());
 ...
 * 
 * Keys will be "k" + integer, values will be the sub-json string: {"key":"value"}
 * yyjson automatically handles escaping
 * 
 * @param dst Output string to store generated JSON
 * @param start Starting value for the sequence
 * @param count Number of key-value pairs to generate
...
void BuildEscapeObject(std::string& dst, int start, int count) {
    ...
    for (int i = 0; i < count; i++) {
        std::string key = "k" + std::to_string(start + i);
        yyjson_mut_val *key_str = yyjson_mut_strncpy(doc, key.c_str(), key.length());
        yyjson_mut_val *val_str = yyjson_mut_strncpy(doc, sub_json.c_str(), sub_json.length());
        yyjson_mut_obj_add(root, key_str, val_str);
    }
...
DEF_TAST(string_array_wwjson, "wwjson 字符串数组构建性能测试")
{
    ...
    for (int i = 0; i < argv.loop; i++) {
        test::wwjson::BuildStringArray(json_data, argv.start, argv.items, argv.size);
    }
    TIME_TOC;
...
DEF_TAST(string_object_wwjson, "wwjson 字符串对象构建性能测试")
{
    ...
    TIME_TIC;
    for (int i = 0; i < argv.loop; i++) {
        test::wwjson::BuildStringObject(json_data, argv.start, argv.items, argv.size);
    }
    TIME_TOC;
...
DEF_TAST(string_escape_wwjson, "wwjson 转义字符串对象构建性能测试")
{
    ...
    TIME_TIC;
    for (int i = 0; i < argv.loop; i++) {
        test::wwjson::BuildEscapeObject(json_data, argv.start, argv.items, argv.size);
    }
    TIME_TOC;
